...
I didn't set eval = FALSE up here in the overall options because I wanted to run tables that would be included in my code. For later chunks that I didn't want to be evaluated, I just added eval = FALSE where necessary.
...
And this is what you would get, ultimately (although each bullet would appear with successive advances):

![](/img/first_slide.png)

If you want to suppress the message of an evaluated chunk:
--
```{r, message = FALSE}
library(tidyverse)
```
...
Okay, that sums it up! Scroll through the mini example slide deck:
```{r}
knitr::include_url('http://aczane.netlify.com/slides/xaringan_example_acz.html')
```
